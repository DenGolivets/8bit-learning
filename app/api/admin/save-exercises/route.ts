...
const DATA = [
  ...
  },
  {
    ...
    exercisesContent: {
      content:
        "<body><h2>Break & Repair</h2><p>Every developer breaks things.</p><p>HTML errors are part of learning.</p><p>Missing closing tags confuse browsers.</p><p>Structure becomes unstable.</p><p>Your job is to fix it.</p><p>Think like a mechanic.</p><p>Open tags must be closed.</p><p>Nested tags must be in order.</p><p>This keeps logic clean.</p><p>Browsers try to guess errors.</p><p>But you should not rely on that.</p><p>Let’s repair the page.</p></body>",
  ...
  },
  {
    ...
    exercisesContent: {
      ...
      regex:
        "<h1>\\s*Element Collection\\s*</h1>[\\s\\S]*<p>\\s*Gather all HTML trasures!\\s*</p>[\\s\\S]*<li>\\s*Headings\\s*</li>[\\s\\S]*<li>\\s*Paragraphs\\s*</li>[\\s\\S]*<li>\\s*Links\\s*</li>",
      output:
        "<h1>Element Collection</h1><p>Gather all HTML trasures!</p><ul><li>Headings</li><li>Paragraphs</li><li>Links</li></ul>",
      hintXp: 50,
    },
  },
];
